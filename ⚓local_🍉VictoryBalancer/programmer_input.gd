...
func parse_virtual_button_input(
	...
	# まず、ボタンの押下状態を確認
	var vk_occurence = self.monkey().input_node().get_occurence(vk_name)
...
func parse_virtual_lever_input(
	...
	# まず、上下を確認
	var down_lever_value = self.monkey().input_node().get_state(&"VK_Down")
	...
	if typeof(target) == TYPE_DICTIONARY:
		...
		# 次に、左右を確認
		var right_lever_value = self.monkey().input_node().get_state(&"VK_Right")
